...
A changeset is a release-notes entry for one user-visible change. Each one bumps `@a2x/sdk` and adds a line to the next release. **Every release-worthy change carries its own changeset** — release notes describe *what changed*, not *which PRs landed*. The changesets bot folds multiple changesets into a single version bump per release, so you don't need to wait for a "release-worthy moment" to add one.

- **Add a changeset when** the change is user-visible. Typical cases:
...
- **Bundling fixes in one PR.** When a single PR contains multiple independent fixes (cherry-picked batch, consolidated review), add **one changeset per fix**, not one omnibus changeset for the PR. Each fix should read as a distinct release-notes entry. The bot rolls them into the same release.
- **Release timing is the maintainer's call, not the changeset author's.** Add the changeset whenever the change is user-visible. The maintainer decides when to cut the release by merging the next `changeset-release/main` PR.
- **Skip the changeset for** internal refactors with no behavior delta, test-only changes, doc-only changes under `packages/a2x/docs/` or `packages/a2x/README.md` (they ride along in the next real release), type tightening with no runtime effect, or dead-code removal.
