...
A changeset does two things: it bumps `@a2x/sdk` on the next release, and it adds an entry to the published CHANGELOG / release notes. Attach one whenever the change belongs on that list.

- **Needs a changeset.** Anything under `packages/a2x/src/` (runtime, public API, types, or refactors that ship in the built artifact); bundled docs under `packages/a2x/docs/`; `packages/a2x/README.md` (shipped as the npm package page); a meaningful `packages/a2x/package.json` shift — new `exports` entry, new peer dependency, Node-version bump, license change.
- **Can skip a changeset.** Pure metadata touch-ups that ride silently on the next real release (adding `author`, tweaking `keywords`, rewording `description`); CI / repo tooling under `.github/` or lint/format configs; `samples/`, `specification/`, and `@a2x/cli`-only changes (CLI is in the changeset `ignore` list and ships via tag-driven GitHub Release).
