...
- **x402 payments** — Charge per call with on-chain cryptocurrency payments, supporting both x402 protocol versions (legacy V1 and the [x402 Foundation](https://github.com/x402-foundation/x402) V2 transport) — each deployment speaks one, V1 by default, V2 via `new X402Context({ x402Version: 2 })`. Compose the `@a2x/sdk/x402` mechanics directly, or opt into its host-neutral `MerchantGate` for shared pricing, metering, frozen terms, and replay protection.
...
For a reusable merchant flow, import `MerchantGate` from `@a2x/sdk/x402`. It returns `request-payment`, `refuse`, and `proceed` outcomes instead of SDK events, so a `BaseAgent.run()` generator and a custom executor can render the same policy differently. Rates, paid/free selection, exact settlement timing, and missing-usage behavior remain required host configuration. See the [x402 guide](./docs/guides/advanced/x402-payments.md#shared-merchant-policy-gate) for the complete example.
