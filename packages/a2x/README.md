...
- **x402 payments** — Charge per call with on-chain cryptocurrency payments, supporting both x402 protocol versions (legacy V1 and the [x402 Foundation](https://github.com/x402-foundation/x402) V2 transport) — each deployment speaks one, V1 by default, V2 via `new X402Context({ x402Version: 2 })`. Compose the `@a2x/sdk/x402` mechanics directly, or opt into its host-neutral `MerchantGate` for shared exact, `upto`, and `batch-settlement` pricing, metering, frozen terms, lifecycle-aware replay protection, and explicit buffered or progressive delivery timing.
