...
One `A2XClient` can be shared by concurrent callers. Cold requests share a single in-flight `AuthProvider.provide()` call, and simultaneous `auth-required` responses share one `refresh()` call. The provider callbacks must not perform an authenticated operation through that same client instance: credential acquisition would otherwise depend on the operation that is waiting for those credentials. The client rejects such re-entry with `UnsupportedOperationError`, including re-entry after an asynchronous boundary. Use a separate client or lower-level transport when credential bootstrap or refresh needs a remote request. Calls from unrelated concurrent code continue to wait for and share the in-flight provider callback.
