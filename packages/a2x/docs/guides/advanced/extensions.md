...
Version is **not** selected by which URI is activated (the canonical URI is version-neutral — see [x402 Payments](./x402-payments.md#protocol-versions-v1--v2)); the server emits the single version its deployment configured. The one signal the channel carries is the legacy v0.2 URI, which declares a V1-only client — whenever it is present, including alongside the foundation URI, an `x402Version: 2` server refuses the activation with `invalid_x402_version` rather than emit envelopes the client cannot decode. Read the activated set inside the agent from `context.activatedExtensions` (see [Manual Wiring](./manual-wiring.md#reading-activated-extensions)).
