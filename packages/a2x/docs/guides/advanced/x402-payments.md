...
| `maxAmount` | no cap | Atomic-unit ceiling per selected requirement, or per new batch deposit. Filters `accepts[]` before the selector runs; it is not an aggregate wallet budget across calls. |
