...
The binding becomes outstanding as soon as the signed submission is handed to the transport. If blocking transport/JSON parsing fails, a non-blocking unary call returns an intermediate task, a follow-up SSE stream errors or is aborted, the caller closes that stream early, or the stream reaches EOF before a receipt or a complete retry prompt, `A2XClient` raises the same quarantine error with `reason: 'ambiguous-response'`. Cancellation detected before transport handoff is different: whether detected while the payment prompt is yielded, after approval, or after signing, it releases the attempt cleanly because the merchant cannot hold the voucher. A status marker alone is not a retry prompt: `x402.payment.required` must also be present before the client treats the previous voucher as rejected. A known intermediate unary response is available as `task`; transport failures and streaming exits have no complete task, and the original transport/stream failure is available as `cause` when one occurred. In every ambiguous case the merchant must be assumed to hold the voucher until an operator reconciles or retires the channel.
...
| `maxAmount` | no cap | Atomic-unit ceiling per selected requirement, or per new batch deposit. Filters `accepts[]` before the selector runs; it is not an aggregate wallet budget across calls. Requirements with a missing amount field or both V1 `maxAmountRequired` and V2 `amount` are rejected according to the envelope's `x402Version`, so policy and signing always inspect the same value. |
