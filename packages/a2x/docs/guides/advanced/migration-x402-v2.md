...
This guide is for projects on `@a2x/sdk` 0.13.x that use `x402PaymentHook` / `inputRoundTripHooks` / `readX402Settlement`. The replacement core surface removes the framework-owned payment flow: agents compose the lifecycle inside `BaseAgent.run()`. Newer releases also provide an optional outcome-based `@a2x/sdk/x402-merchant` gate, but it does not register hooks or choose merchant policy implicitly.
...
The new design removes the framework-installed flow entirely. The core surface exposes separate mechanics the agent composes inside `run()` — every step is callable in order with user logic inserted between them. The optional `@a2x/sdk/x402-merchant` subpath is an explicit composition helper that returns outcomes instead of scheduling events.
