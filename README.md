...
- **Shared x402 merchant policy** — Optional `MerchantGate` from `@a2x/sdk/x402` for hosts that want reusable exact, `upto`, and `batch-settlement` pricing, metering, frozen offer terms, and lifecycle-aware replay protection without coupling the SDK to `AgentEvent`s or hard-coded rates.
